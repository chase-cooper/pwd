...
### Applying COS line spread functions
...
### Retrieve model and observed spectra from provided files
...
def trimModel(wvln,flux):
    """
    Trim model spectrum to the range 900 to 2000AA
    """
    new_wvln,new_flux = [],[]
    ...
    return new_wvln,new_flux

def openSpectrum(fname:str):
    """
...
### Convolve model spectra with appropriate line spread function
...
def convolveModel(fnamem,fname):
    ...
    wvln_m,spec_m = trimModel(wvln_m,spec_m)
...
def scaleModel(wvln,flux,sigma,wvln_m,flux_m,plotit:bool=False):
    """
    Fit an observed spectrum (or stacked spectrum) with a modeled one, adjusting for a scale
    factor and Doppler shift. The modeled spectrum must already be convolved with the LSF.
    """
    ...
    return scaled_model_wvln,scaled_model_flux,chi2_grid[ind_v][ind_s]

##########################################################################################

### Produce a range of models with varying concentrations in one particular species

def varySpeciesConcentration(scen_name:str,teff,logg,sp:str,lo=-9,hi=-4,ddex=0.5,params={}):
    """
    Produce a range of TLUSTY models with the same values for T_eff and log(g), but with varying
    concentrations of a provided species.

    This process can take some time

    Inputs:
    - scen (string):        the scenario name, controls where the outputs will be saved
    - teff (int/float):     effective temperature
    - logg (int/float):     log(g)
    - sp (string):          species whose concentrations will vary
    - lo (int/float):       lower bound of log(Z/H)
    - hi (int/float):       upper bound of log(Z/H)
    - ddex(int/float):      spacing between values of log(Z/H)
    - other (dict[str]):    other parameters to pass to the tlusty.py call, namely other species'
                                abundances
    ...
    while conc <= hi:
        conc_str    = str(conc)
        command     = ['python','tlusty.py',scen_name+f'_{sp}={conc_str}',tstr,gstr]
        for param in params:
            command.append(param)
        command.append(f"{sp}={conc_str}!")
        subprocess.run(command)
        conc += ddex

def lineFit(fname_spec,scen_name,teff,logg,linecenter,width,sp,lo,hi,ddex,params):
    """
    Fit for the abundance of a given species using SYNSPEC spectra.

    Inputs:
    - fname_spec (string):      HST COS file whose data is being fit
    - scen_name (string):       name for the models
    - teff (int/float):         effective temperature of the models.
    - logg (int/float):         log(g) of the models
    - linecenter (int/float):   center of spectral range to fit, in AA
    - width (int/float):        width of spectral range in either direction, in AA
    - sp (string):              species whose concentration is varying
    - lo (int/float):           lower bound on log(Z/H)
    - hi (int/float):           upper bound on log(Z/H)
    - ddex (int/float):         size of log(Z/H) steps
    - params (dict[string]):    additional parameters to pass to tlusty.py call
    """

    # Produce models with varied species concentrations
    varySpeciesConcentration(scen_name=scen,
                             teff=teff,
                             logg=logg,
                             sp=sp,
                             lo=lo,
    ...
    # Get observed spectrum and trim
    wvln_spec,flux_spec,sigma_spec = openSpectrum(fname_spec)

    wvln_s,flux_s,sigma_s   = [],[],[]
    for i in range(len(wvln_spec)):
        w = wvln_spec[i]
        if (w > linecenter-width) and (w < linecenter+width):
            wvln_s.append(w)
            flux_s.append(flux_spec[i])
            sigma_s.append(sigma_spec[i])

    wvln_s = np.array(wvln_s)
    ...
    while conc <= hi:
        conc_str    = str(conc)
        model_name  = f"{scen_name}_{sp}={conc_str}"
        model_file  = f"tlusty/scenarios/{model_name}/synspec/fort.7"
        wvln_model,flux_model   = convolveModel(model_file,fname_spec)

        # Trim model spectrum. Leave 2% wiggle room to account for Doppler shift
        wvln_m,flux_m = [],[]
        for i in range(len(wvln_model)):
                w = wvln_model[i]
                if (w > (linecenter-width)*0.98) and (w < (linecenter+width)*1.02):
                    wvln_m.append(w)
                    flux_m.append(flux_model[i])

        wvln_m = np.array(wvln_m)
        flux_m = np.array(flux_m)

        print(model_name)
        wvln_m,flux_m,chi2_m = scaleModel(wvln_s,flux_s,sigma_s,wvln_m,flux_m)
        concentrations.append(conc)
        chi2_scores.append(chi2_m)

        conc += ddex

    ### Calculate errorbars on concentration
    likelihood  = np.exp(-0.5*np.array(chi2_scores))
    pdf         = likelihood / np.sum(likelihood)       # probability distribution function
    cdf         = np.zeros((len(pdf)))                  # cumulative distribution function
    for i in range(len(pdf)):
        cdf[i]  = np.sum(pdf[:i+1])
    print(pdf)
    print(cdf)

fname_spec   = 'fits files/WD1226+110_1.fits'
scen = 'wd1226+110'
params = {
    'c=-6!'
}

lineFit(fname_spec=fname_spec,
        scen_name=scen,
        teff=22840,
        logg=7.86,
        linecenter=1253.8,
